...
export const useDocumentosStore = defineStore('documentos', () => {
  ...
  /** EXS-3 — los adjuntos de un anuncio. Misma forma que las fotos de una publicación y por el
   * mismo motivo: se esperan VARIAS filas, no la versión vigente de un grupo. */
  async function cargarAdjuntosAnuncio(
    tenantId: string,
    anuncioId: string,
  ): Promise<DocumentoVigenteRow[]> {
    loading.value = true
    try {
      const cliente = useSupabaseClient<Database>()
      const { data, error: errorAdjuntos } = await cliente
        .from('v_documento_vigente')
        .select('*')
        .eq('tenant_id', tenantId)
  ...
  /** Fase 5 de mantenimiento de activos (D-92) — las fotos de un activo físico. Misma forma que
   * las demás galerías (EXS-3/4/6): se esperan VARIAS filas, no la versión vigente de un grupo. */
  async function cargarImagenesActivo(
    tenantId: string,
    activoId: string,
  ): Promise<DocumentoVigenteRow[]> {
    loading.value = true
    try {
      const cliente = useSupabaseClient<Database>()
      const { data, error: errorImagenes } = await cliente
        .from('v_documento_vigente')
        .select('*')
        .eq('tenant_id', tenantId)
  ...
  async function subirDocumento(params: {
    ...
    publicacionId?: string
    /** EXS-3: adjunto de una comunicación oficial. Como la publicación, un anuncio tiene VARIOS
