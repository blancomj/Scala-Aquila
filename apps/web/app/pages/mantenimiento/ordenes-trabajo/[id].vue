...
async function cargar(): Promise<void> {
  ...
  await Promise.all([
    ...
    tiposEvidencia.value.length === 0
      ? cargarListaTipos(tenantId, 'EVIDENCIA_OT_TIPO').then((d) => { tiposEvidencia.value = d })
      : Promise.resolve(),
  ...
  if (activoId) {
    ...
    const { data: activo } = await cliente.from('activos').select('tipo_id').eq('id', activoId).single()
    if (activo) {
      const { data: defs } = await cliente
        .from('mant_atributo_definicion').select('*')
        .eq('tenant_id', tenantId).eq('tipo_activo_id', activo.tipo_id).eq('tipo_dato', 'numero')
        .order('orden')
...
async function cancelar(): Promise<void> {
  ...
  errorAccion.value = null
  try {
    await ordenesStore.actualizarOt(ot.value.id, { estado: 'cancelada', cancelada_motivo: motivoCancelar.value.trim() })
...
async function marcarTareaEjecutada(tareaId: string): Promise<void> {
  ...
  errorAccion.value = null
  try {
    await ordenesStore.actualizarTarea(tareaId, ot.value.id, { estado: 'ejecutada', ejecutada_at: new Date().toISOString() })
  } catch (excepcion) {
...
async function marcarTareaNoAplica(tareaId: string): Promise<void> {
  ...
  errorAccion.value = null
  try {
    await ordenesStore.actualizarTarea(tareaId, ot.value.id, { estado: 'no_aplica', no_aplica_motivo: motivo })
...
const formTarea = reactive({ descripcion: '', obligatoria: true, requiereMedicion: false, requiereEvidenciaFoto: false })
async function agregarTareaManual(): Promise<void> {
  ...
  try {
    await ordenesStore.agregarTarea({
      tenant_id: ot.value.tenant_id, ot_id: ot.value.id, orden: ordenesStore.tareas.length + 1,
      descripcion: formTarea.descripcion.trim(), obligatoria: formTarea.obligatoria,
      requiere_medicion: formTarea.requiereMedicion, requiere_evidencia_foto: formTarea.requiereEvidenciaFoto,
    })
...
async function agregarMedicion(): Promise<void> {
  ...
  try {
    await ordenesStore.agregarMedicion({
      tenant_id: ot.value.tenant_id, ot_id: ot.value.id, tarea_id: formMedicion.tareaId ?? null,
      atributo_definicion_id: formMedicion.atributoDefinicionId, valor: formMedicion.valor,
      rango_min: formMedicion.rangoMin, rango_max: formMedicion.rangoMax,
    })
...
const formEvidencia = reactive({ tareaId: undefined as string | undefined, tipoEvidenciaId: undefined as number | undefined })
...
async function agregarEvidencia(): Promise<void> {
  ...
  try {
    ...
    if (!tipoEvidenciaDocumento) throw new Error('No se encontró el tipo de documento "evidencia_ot".')
    const documento = await documentosStore.subirDocumento({
      tenantId, inmuebleId: null, tipoDocumentoId: tipoEvidenciaDocumento.id, archivo: archivoEvidencia.value,
    })
    if (!documento.id) throw new Error('El documento subido no devolvió id.')
    await ordenesStore.agregarEvidencia({
      tenant_id: tenantId, ot_id: ot.value.id, tarea_id: formEvidencia.tareaId ?? null,
      documento_id: documento.id, tipo_evidencia_id: formEvidencia.tipoEvidenciaId ?? null,
    })
...
<template>
  <div v-if="ot" class="space-y-6">
    <div class="flex items-start justify-between gap-4 flex-wrap">
      <div>
        <UButton variant="link" icon="i-lucide-arrow-left" to="/mantenimiento/ordenes-trabajo" class="px-0 mb-1">
          Órdenes de trabajo
        </UButton>
        <h1 class="text-lg sm:text-xl font-semibold">{{ ot.anio }}-{{ ot.numero }} · {{ ot.titulo }}</h1>
        <p class="text-sm text-muted">{{ nombreTipo.get(ot.tipo_mantenimiento_id) ?? '—' }} · origen {{ ot.origen }}</p>
      </div>
      <UBadge :color="ESTADO_COLOR[ot.estado] ?? 'neutral'" variant="soft" size="lg" class="capitalize">
        {{ ot.estado.replace('_', ' ') }}
      </UBadge>
    </div>
    ...
    <NuxtLink v-if="ot.incidencia_id" :to="`/mantenimiento/incidencias/${ot.incidencia_id}`" class="text-sm text-primary underline">
    ...
    <section class="rounded-lg border border-default p-4 flex flex-wrap gap-2">
      <UButton v-if="ot.estado === 'borrador'" @click="transicionar('programada')">Programar</UButton>
      <UButton v-if="['borrador', 'programada'].includes(ot.estado)" @click="transicionar('asignada')">Asignar</UButton>
      <UButton v-if="ot.estado === 'asignada'" @click="transicionar('en_ejecucion')">Iniciar ejecución</UButton>
      <UButton v-if="ot.estado === 'en_ejecucion'" color="success" @click="transicionar('ejecutada')">Marcar ejecutada</UButton>
      <UButton v-if="ot.estado === 'ejecutada' && ot.requiere_aprobacion" @click="transicionar('pendiente_aprobacion')">
        Enviar a aprobación
      </UButton>
      <UButton v-if="['ejecutada', 'pendiente_aprobacion'].includes(ot.estado)" color="success" icon="i-lucide-check" @click="abrirCerrar()">
        Cerrar OT
      </UButton>
      <UButton
        v-if="!['cerrada', 'cancelada'].includes(ot.estado)" variant="soft" color="error"
        @click="motivoCancelar = ''; drawerCancelar = true"
      >
        Cancelar
      </UButton>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-6">
            ...
            <div v-for="t in ordenesStore.tareas" :key="t.id" class="py-3 space-y-2">
              <div class="flex items-start justify-between gap-3 flex-wrap">
                ...
                <UBadge
                  ...
                  :color="t.estado === 'ejecutada' ? 'success' : t.estado === 'no_aplica' ? 'neutral' : 'warning'"
                >
                  {{ t.estado.replace('_', ' ') }}
                </UBadge>
              </div>
              <div v-if="t.estado === 'pendiente' && ot.estado !== 'cerrada'" class="flex items-center gap-2 flex-wrap">
                <UButton size="sm" @click="marcarTareaEjecutada(t.id)">Marcar ejecutada</UButton>
                <UInput v-model="motivoNoAplicaPorTarea[t.id]" size="sm" placeholder="Motivo de no aplica" class="w-48" />
                <UButton size="sm" variant="soft" :disabled="!motivoNoAplicaPorTarea[t.id]?.trim()" @click="marcarTareaNoAplica(t.id)">
                  No aplica
                </UButton>
              </div>
            </div>
            <p v-if="ordenesStore.tareas.length === 0" class="py-4 text-sm text-muted text-center">Sin tareas.</p>
          </div>
          <div v-if="ot.estado !== 'cerrada'" class="flex flex-wrap items-end gap-2 pt-2 border-t border-default">
            <UInput v-model="formTarea.descripcion" placeholder="Nueva tarea…" class="flex-1 min-w-40" />
            <label class="flex items-center gap-1 text-xs text-muted"><UCheckbox v-model="formTarea.obligatoria" />Obligatoria</label>
            <label class="flex items-center gap-1 text-xs text-muted"><UCheckbox v-model="formTarea.requiereMedicion" />Medición</label>
            <label class="flex items-center gap-1 text-xs text-muted"><UCheckbox v-model="formTarea.requiereEvidenciaFoto" />Foto</label>
            <UButton size="sm" :loading="ordenesStore.guardando" :disabled="!formTarea.descripcion.trim()" @click="agregarTareaManual()">
              Agregar tarea
            </UButton>
          </div>
        </section>
        ...
        <!-- Mediciones -->
        <section v-if="definicionesAtributo.length > 0 || ordenesStore.mediciones.length > 0" class="rounded-lg border border-default p-4 space-y-3">
          <h2 class="font-medium">Mediciones</h2>
          <div class="divide-y divide-default">
            <div v-for="m in ordenesStore.mediciones" :key="m.id" class="py-2 text-sm flex items-center justify-between gap-3">
              <span>{{ nombreDefinicion.get(m.atributo_definicion_id)?.nombre ?? '—' }}: {{ m.valor }}</span>
              <UBadge v-if="m.fuera_de_rango" color="error" variant="soft" size="sm">Fuera de rango</UBadge>
            </div>
            <p v-if="ordenesStore.mediciones.length === 0" class="py-2 text-sm text-muted">Sin mediciones registradas.</p>
          </div>
          <div v-if="definicionesAtributo.length > 0 && ot.estado !== 'cerrada'" class="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-default">
            ...
            <UButton class="col-span-full" :loading="ordenesStore.guardando" :disabled="!formMedicion.atributoDefinicionId || formMedicion.valor === null" @click="agregarMedicion()">
              Registrar medición
            </UButton>
          </div>
        </section>
        ...
        <section class="rounded-lg border border-default p-4 space-y-3">
          ...
          <div class="divide-y divide-default">
            <div v-for="e in ordenesStore.evidencias" :key="e.id" class="py-2 text-sm">
              {{ tiposEvidencia.find((t) => t.id === e.tipo_evidencia_id)?.nombre ?? 'Documento adjunto' }}
            </div>
            <p v-if="ordenesStore.evidencias.length === 0" class="py-2 text-sm text-muted">Sin evidencias adjuntas.</p>
          </div>
          <div v-if="ot.estado !== 'cerrada'" class="flex flex-wrap items-center gap-2 pt-2 border-t border-default">
            <input type="file" accept=".pdf,.jpg,.jpeg,.png" class="text-sm" @change="elegirArchivo">
            <USelect
              v-model="formEvidencia.tareaId"
              :items="[{ label: 'General', value: undefined }, ...tareasQueMidenOEvidencian.filter((t) => t.requiere_evidencia_foto).map((t) => ({ label: t.descripcion, value: t.id }))]"
              class="w-40"
            />
            <USelect v-model="formEvidencia.tipoEvidenciaId" :items="tiposEvidencia.map((t) => ({ label: t.nombre, value: t.id }))" class="w-40" />
            <UButton size="sm" :loading="documentosStore.subiendo" :disabled="!archivoEvidencia" @click="agregarEvidencia()">
              Subir
            </UButton>
          </div>
        </section>
        ...
        <section class="rounded-lg border border-default p-4 space-y-2">
          ...
          <div class="divide-y divide-default">
            <div v-for="h in ordenesStore.historial" :key="h.id" class="py-2 text-sm">
              ...
              <p>{{ h.estado_anterior ?? '—' }} → {{ h.estado_nuevo }}<span v-if="h.motivo"> · {{ h.motivo }}</span></p>
            </div>
            <p v-if="ordenesStore.historial.length === 0" class="py-4 text-sm text-muted text-center">Sin transiciones aún.</p>
          </div>
        </section>
      </div>
      ...
      <div class="space-y-3">
        <section class="rounded-lg border border-default p-4 space-y-3">
          ...
          <UFormField label="Referencia de acreditación" name="acreditacion">
            <UInput v-model="asignacion.acreditacionReferencia" class="w-full" placeholder="ej. certificado técnico N°..." />
          </UFormField>
          <UFormField label="Costo estimado" name="costo">
            <UInput v-model.number="asignacion.costoEstimado" type="number" min="0" class="w-full" />
          </UFormField>
          <UButton block :loading="ordenesStore.guardando" @click="guardarAsignacion()">Guardar</UButton>
        </section>
      </div>
    </div>
    ...
    <UiDrawer :abierto="drawerCancelar" titulo="Cancelar OT" @cerrar="drawerCancelar = false">
      ...
      <template #foot>
        <div class="flex justify-end gap-2 w-full">
          ...
          <UButton color="error" :disabled="!motivoCancelar.trim()" :loading="ordenesStore.guardando" @click="cancelar()">
            Cancelar OT
          </UButton>
        </div>
      </template>
    ...
    <UiDrawer :abierto="drawerCerrar" titulo="Cerrar orden de trabajo" @cerrar="drawerCerrar = false">
      <div class="space-y-3">
      ...
      <template #foot>
        <div class="flex justify-end gap-2 w-full">
          ...
          <UButton
            color="success" :loading="ordenesStore.guardando"
