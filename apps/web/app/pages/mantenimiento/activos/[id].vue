...
// ── MANT-6 §4.4: costos — SIEMPRE leídos de mant_costos() (presupuesto_ejecucion), nunca
// calculados aparte. Rango por defecto: año en curso. ──
const hoy = new Date()
...
async function cargar(): Promise<void> {
  ...
  await Promise.all([
    ...
    unidades.value.length === 0
      ? cargarListaTipos(tenantId, 'UNIDAD_MEDIDA').then((data) => {
          unidades.value = data
        })
      : Promise.resolve(),
    cargarCostos(),
...
<template>
  <div class="space-y-6">
    ...
    <section class="space-y-4 rounded-lg border border-default p-4">
      ...
      <p v-if="activosStore.definiciones.length === 0" class="text-sm text-muted">
        ...
        <NuxtLink to="/mantenimiento/configuracion" class="text-primary underline"
          >configúralos aquí</NuxtLink
        >.
      </p>
      <div v-else class="grid sm:grid-cols-2 gap-4">
        <UFormField
          v-for="def in activosStore.definiciones"
          :key="def.id"
          :label="`${def.nombre}${def.unidad_id ? ` (${nombreUnidad.get(def.unidad_id) ?? ''})` : ''}${def.obligatorio ? ' *' : ''}`"
          :name="def.codigo"
        >
          <UInput
            v-if="def.tipo_dato === 'numero'"
            v-model.number="form[def.codigo]"
            type="number"
          />
      ...
      <UButton
        v-if="activosStore.definiciones.length > 0"
        :loading="activosStore.guardando"
    ...
    <section class="space-y-4 rounded-lg border border-default p-4">
      ...
      <UAlert
        v-else-if="activosStore.errorCriticidad"
        color="warning"
        variant="soft"
      ...
      <div v-if="activosStore.criteriosVigentes.length > 0" class="space-y-3">
        <div
          v-for="c in activosStore.criteriosVigentes"
          :key="c.id"
          class="flex items-center justify-between gap-4 flex-wrap"
        >
          <div>
            <p class="text-sm font-medium">
              {{ c.nombre }} <span class="text-muted">(peso {{ c.peso }})</span>
            </p>
          </div>
          <USelect
            :model-value="valorEvaluado(c.id)"
            :items="
              Object.keys(c.escala as Record<string, number>).map((k) => ({ label: k, value: k }))
            "
    ...
    <!-- MANT-6 §4.4: costos — SIEMPRE desde mant_costos() (presupuesto_ejecucion), nunca aparte -->
    <section class="space-y-4 rounded-lg border border-default p-4">
      ...
      <p class="text-xs text-muted">
        Costo real acumulado en el rango, leído de la ejecución presupuestal — no incluye el costo
        estimado de ninguna OT (una estimación nunca sustituye al gasto real).
      </p>
    </section>
  </div>
</template>
