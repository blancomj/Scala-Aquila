...
| **GAP-19** | Reapertura de "todo agrega en un único concepto `CUOTA_ADMIN`" (línea 424) — motivada por `Docs/Motor presupuestal/AQUILA_SAAS_E16_...md`: arts. 25/31 Ley 675 exigen destinación sectorial y fuentes de financiación distintas de la cuota, algo irrepresentable con un solo concepto | Motor Presupuestal | **Adoptado, físico parcial**: `fuente_financiacion`/`fundamento_normativo` implementadas (20260814200000_motor_presupuestal_financiacion.sql, tests/rls/motor-presupuestal-financiacion.test.ts). Pendiente: neteo contra `monto_total`/`conceptos.CUOTA_ADMIN` (deliberadamente diferido, ver cabecera de la migración) y capa de exposición (RPC/Edge Function/UI) |
...
son alcance diferido, no ambigüedad. GAP-19 queda adoptado (decisión tomada) con
