...
- **Codificación por origen del dato en el constructor de fórmulas** (`components/AelBlock*.vue`, Fase 8 mov. 10): las píldoras de una expresión se colorean según de dónde sale el dato — Parámetro (emerald), Inmueble (sky), Concepto (violet), Función (amber). Es categórico, igual que dataviz: distingue cuatro orígenes que el usuario necesita separar de un vistazo, no jerarquiza interacción. Reglas que sí aplican: la paleta se declara **una sola vez** y vale para los dos modos del editor (píldoras del lienzo y tema de CodeMirror; antes no coincidían en nada), el lienzo lleva **leyenda visible**, y ningún color categórico puede repetirse en un contenedor estructural — el contenedor del condicional era morado, el mismo de Concepto, y pasó a `neutral-*` por esa colisión. El acento `primary` sigue reservado a **interacción** (foco, botones, estados activos) y nunca marca estructura: las guías de sangría de las ramas del condicional son `neutral-*`, y las dos ramas se distinguen por su etiqueta («Entonces» / «De lo contrario»), no por color.
