...
- **Codificación por origen del dato en el editor de fórmulas** (Fase 8): un valor de una fórmula se colorea según **de dónde sale**, no según qué clase de token es. Cuatro orígenes: Parámetro, Inmueble, Concepto y Función. Es categórico igual que dataviz — separa cosas que el usuario necesita distinguir de un vistazo, no jerarquiza interacción.
  ...
  Los valores viven **en un solo sitio**, `apps/web/app/assets/css/tokens.css`, como variables `--ael-parametro`, `--ael-inmueble`, `--ael-concepto`, `--ael-funcion` (cada una con su `-fondo` y `-borde`, y su bloque `.dark`). Las consumen los **dos modos** del editor y literalmente las mismas variables: las píldoras del lienzo vía `:style` (`AelBlockExpresion`, `AelBlockCanvas`) y el resaltado de sintaxis vía el tema de CodeMirror (`utils/ael-codemirror.ts`). Por eso el resaltado del modo texto mira el contexto del token —el nombre del contrato, su campo y las llamadas a función— en vez de colorear solo por tipo de token: sin eso, `PARAMETER.X` y `CONCEPTO.X` se verían iguales en texto y distintos en bloques.

  Tres reglas que se derivan de ahí. **Lo estructural va en neutro en los dos modos** (`--ael-estructura`): palabras clave, operadores, puntuación y las etiquetas de los bloques — antes las palabras clave eran moradas, el mismo morado que Concepto. **Ningún color categórico se repite en un contenedor**: el contenedor del condicional era morado y pasó a `neutral-*`. Y el lienzo lleva **leyenda visible**, porque un código de color sin leyenda es adivinanza.

  El acento `primary` sigue reservado a **interacción** (foco, botones, estados activos) y nunca marca estructura: las guías de sangría de las ramas son `neutral-*`, y las ramas se distinguen por su etiqueta («Entonces» / «De lo contrario»), no por color.
