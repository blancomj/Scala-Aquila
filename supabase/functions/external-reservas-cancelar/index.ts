// EXT-03 §3.3 · El actor externo cancela su propia reserva mientras siga en solicitada o
// aprobada. fn_reserva_cancelar_externa ya valida internamente que el vínculo pertenece al
...
// que el resto de las Edge Functions de este corte (contrato uniforme, mismo criterio que
// external-solicitudes-cancelar de EXT-02).
...
const ESTADO_HTTP_POR_CODIGO: Record<string, number> = {
  RESERVA_TRANSICION_INVALIDA: 409,
...
Deno.serve(async (req) => {
  ...
  const cuerpo = body as { vinculo_id?: unknown; reserva_id?: unknown } | null
  const vinculoId = cuerpo?.vinculo_id
  const reservaId = cuerpo?.reserva_id
  if (typeof vinculoId !== 'string' || typeof reservaId !== 'string') {
    return errorResponse(400, 'INVALID_PAYLOAD', 'vinculo_id y reserva_id son requeridos.', undefined, correlationId)
  }
  ...
  const bloqueo = await enforceRateLimit(
    cliente, `reservas_cancelar_vinculo:${vinculo.vinculo_id}`, RATE_LIMIT_MAX_HITS, RATE_LIMIT_VENTANA, correlationId,
  )
  if (bloqueo) return bloqueo

  const { data: reserva, error: errorCancelar } = await cliente
    .rpc('fn_reserva_cancelar_externa', { p_vinculo_id: vinculo.vinculo_id, p_reserva_id: reservaId })
    .single()
  if (errorCancelar) {
  ...
  return jsonResponse(reserva, 200, correlationId)
})
