// E4 — aprueba una novedad y materializa su cargo (AD-33). novedades no
// tiene política UPDATE para `authenticated` (aprobar exige UPDATE+INSERT
// atómico en fn_aprobar_novedad, 20260816110000/20260816110100) — mismo
// razonamiento de bypass que liquidar-periodo con guardarLiquidacion: el
// rol se verifica aquí explícitamente antes de usar ctx.supabaseAdmin.
...
export default {
  fetch: withSupabase<Database>({ auth: 'user' }, async (req, ctx) => {
    ...
    const { data: aprobada, error: errorRpc } = await ctx.supabaseAdmin
      .rpc('fn_aprobar_novedad', { p_novedad_id: novedadId, p_actor_id: actorId })
      .single()
    if (errorRpc) {
      const { code, message } = parsearErrorRpc(errorRpc.message)
      const status =
        code === 'PERIODO_NO_ENCONTRADO_PARA_FECHA_EFECTIVA'
          ? 422
      ...
      logEvent({
        level: 'warn',
        action: 'aprobar_novedad.rpc_error',
        correlationId,
        actorId,
        meta: { code },
      })
    ...
    logEvent({
      level: 'info',
      action: 'aprobar_novedad.completada',
      correlationId,
      actorId,
