// EXT-04 §3.2 · El actor externo revoca su propia autorización mientras siga vigente.
// fn_autorizacion_visita_revocar_externa ya valida internamente que el vínculo pertenece al
// auth.uid() que llama — aquí se hace el mismo chequeo explícito contra
// fn_actor_externo_mis_vinculos ANTES, para devolver siempre el mismo 403 VINCULO_NO_PERTENECE
// que el resto de las Edge Functions de esta serie (contrato uniforme).
...
const ESTADO_HTTP_POR_CODIGO: Record<string, number> = {
  AUTORIZACION_ESTADO_INMUTABLE: 409,
  AUTORIZACION_INEXISTENTE: 404,
}
...
Deno.serve(async (req) => {
  ...
  const cuerpo = body as { vinculo_id?: unknown; autorizacion_id?: unknown } | null
  const vinculoId = cuerpo?.vinculo_id
  const autorizacionId = cuerpo?.autorizacion_id
  if (typeof vinculoId !== 'string' || typeof autorizacionId !== 'string') {
    return errorResponse(400, 'INVALID_PAYLOAD', 'vinculo_id y autorizacion_id son requeridos.', undefined, correlationId)
  }
  ...
  const bloqueo = await enforceRateLimit(
    cliente, `visitas_revocar_vinculo:${vinculo.vinculo_id}`, RATE_LIMIT_MAX_HITS, RATE_LIMIT_VENTANA, correlationId,
  )
  if (bloqueo) return bloqueo

  const { data: autorizacion, error: errorRevocar } = await cliente
    .rpc('fn_autorizacion_visita_revocar_externa', { p_vinculo_id: vinculo.vinculo_id, p_autorizacion_id: autorizacionId })
    .single()
  if (errorRevocar) {
  ...
  return jsonResponse(autorizacion, 200, correlationId)
})
