...
// (Modelo §20, D-37). Mismo razonamiento que fondos-aprobar-solicitud:
// guard_fondo_solicitud_uso_transicion ya exige rol, no-autoaprobación y
// motivo_rechazo vía trigger sobre el UPDATE directo que la RLS ya
// permite — esta función solo agrega rate limit, error estructurado y
// logging sobre el propio ctx.supabase (RLS) del usuario.
...
const payloadSchema = z.object({
  ...
  motivo_rechazo: z.string().trim().min(1, 'motivo_rechazo es obligatorio para rechazar.'),
})
...
const ESTADO_HTTP_POR_CODIGO: Record<string, number> = {
  ...
  SOLICITUD_MOTIVO_REQUERIDO: 422,
}
...
export default {
  fetch: withSupabase<Database>({ auth: 'user' }, async (req, ctx) => {
    ...
    const { solicitud_id: solicitudId, motivo_rechazo: motivoRechazo } = parseo.data

    const bloqueo = await enforceRateLimit(
      ctx.supabase,
      `fondos_rechazar_solicitud:${actorId}`,
      RATE_LIMIT_MAX_HITS,
      RATE_LIMIT_VENTANA,
      correlationId,
    )
    ...
    const { data: rechazada, error: errorUpdate } = await ctx.supabase
      .from('fondo_solicitudes_uso')
    ...
    if (errorUpdate) {
      ...
      logEvent({
        level: 'warn',
        action: 'fondos_rechazar_solicitud.rechazada',
        correlationId,
        actorId,
    ...
    logEvent({
      level: 'info',
      action: 'fondos_rechazar_solicitud.completada',
      correlationId,
      actorId,
      tenantId: solicitud.tenant_id,
