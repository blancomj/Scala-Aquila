-- ═══════════════════════════════════════════════════════════════════════
--  Descripción real de cada movimiento en vez de la categoría genérica
--  (petición del usuario, 2026-08-27, tras ver "Otro" x4 indistinguibles
--  para 4 novedades distintas en la prueba de intereses).
--
--  · categoria='otro' (novedad) → novedades.descripcion ("Sanción por
--    ruido nocturno") en vez de "Otro". Cae a 'Otro' solo si el cargo no
--    tiene novedad_id (no debería pasar hoy — cargos_origen_unico exige
--    novedad_id para origen_tipo='novedad' — pero el fallback es gratis).
--  · categoria='capital'/'interes' → agrega el código del concepto
--    ("Capital · ADMINISTRACION") cuando existe — mismo criterio que ya
--    usaba cuentaCorriente.ts::generarEstadoCuenta (la ruta manual), que
--    el emisor por lote nunca replicó. Se pareja aquí.
-- ═══════════════════════════════════════════════════════════════════════
...
comment on function public.fn_emitir_estados_cuenta(uuid) is
  'Emite el estado de cuenta de todas las unidades activas del tenant (L5). Descripción de cada '
  'movimiento: capital/interés con código de concepto cuando existe, novedades con su descripción '
  'real (20260902150000, antes "Otro" genérico). saldo_anterior por periodo del cargo, no por '
  'created_at (20260902140000). Resto igual a 20260902130000. Idempotente por liquidación.';
