-- ═══════════════════════════════════════════════════════════════════════
--  GOB-3 · fix — gobierno_quorum() declaraba v_tenant_id y nunca la usaba
--  (usaba r.tenant_id directamente en la subconsulta). `supabase db lint`
--  lo señaló como variable no leída; se corrige reutilizándola.
-- ═══════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_organo_tipo_codigo = 'consejo_administracion' then
    ...
    left join public.coeficiente_sets cs
      on cs.id = coalesce(r.coeficiente_set_id, public.fn_coeficiente_set_vigente(v_tenant_id, v_fecha))
    where r.id = p_reunion_id;
