-- ═══════════════════════════════════════════════════════════════════════
--  CO-3 · Corrección — `origen_entidad`/`origen_id` como nombres de columna
--  de retorno de fn_contabilizar_periodo colisionaban con las columnas
--  homónimas de contable_comprobante en cada WHERE/INSERT del cuerpo:
--  PL/pgSQL trata los OUT params de una función RETURNS TABLE como
--  variables visibles en todo el cuerpo, así que una referencia sin
--  calificar a `origen_entidad`/`origen_id` es ambigua entre la variable y
--  la columna real. `supabase db lint` lo detectó tras aplicar
--  20260930250000 (sqlState 42702, esta vez sobre `origen_entidad`).
--
--  En vez de calificar cada referencia una por una (frágil: la próxima
--  columna nueva del cuerpo puede volver a colisionar), se renombran los
--  OUT params a `hecho_entidad`/`hecho_id`, que no coinciden con ninguna
--  columna real de las tablas que la función toca. Ningún cambio de
--  comportamiento — solo nombres de columna del resumen.
-- ═══════════════════════════════════════════════════════════════════════

-- create or replace no permite cambiar la forma de las columnas OUT de una función ya existente
-- (SQLSTATE 42P13) — hay que soltarla primero.
drop function public.fn_contabilizar_periodo(uuid, uuid);
