-- ═══════════════════════════════════════════════════════════════════════
--  MOV-1 · El visitante que se pasó del tiempo entra en la bandeja
--
--  `movilidad_config.horas_max_visitante` define cuánto puede quedarse un
--  vehículo de visitante. Excederse no lo expulsa —esto no levanta
--  talanqueras— pero SÍ es trabajo de alguien: llamar al apartamento,
--  pedir que lo muevan. Por eso va a "Mis asuntos" y no a una
--  notificación: una notificación se lee y desaparece; un asunto sigue
--  ahí hasta que el carro se va.
--
--  Y se va SOLO, sin que nadie lo cierre: la rama se apoya en
--  `fn_movilidad_dentro`, que deriva del último paso de cada placa. En
--  cuanto la portería registra la salida, el asunto deja de cumplir el
--  where y desaparece. Es la misma propiedad que hace que esta bandeja
--  no necesite tabla (EXS-7).
--
--  Octava rama. Se recrea la función entera con DROP + CREATE citando la
--  firma exacta: `create or replace` bastaría porque el tipo de retorno
--  no cambia, pero el drop explícito es la costumbre que impide dejar un
--  overload huérfano, que es el error que este repositorio ya cometió al
--  ampliar una función con un parámetro nuevo.
-- ═══════════════════════════════════════════════════════════════════════
...
comment on function public.fn_mis_asuntos is
  ...
  'asunto de un auxiliar que no puede aprobarlo. La octava rama (MOV-1) trae al visitante que se '
  'pasó del tiempo, y desaparece sola cuando la portería registra su salida. `asignado_a` (20260933800000) solo lo trae la rama '
